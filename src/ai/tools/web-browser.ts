...
/**
 * @fileOverview A tool for fetching content from a web page using a public search engine.
 */
...
export async function getPageContentFn(input: { query: string }) {
  try {
    const url = `https://searx.be/search?q=${encodeURIComponent(input.query)}&format=json`;
